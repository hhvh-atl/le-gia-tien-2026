...
type Data = { user: { email: string; displayName: string }; admin: Admin; authKind: "group" | "chatgpt"; registrations: Row[]; administrators: Admin[] };
...
export default function Dashboard() {
  ...
  async function signOut() { if (data?.authKind === "group") { await fetch("/api/group-login", { method: "DELETE" }); window.location.href = "/login"; } else { window.location.href = "/signout-with-chatgpt?return_to=/"; } }
  function exportCsv() { const header = ["Tên thân hữu", "Tên thân nhân quá cố", "Mối quan hệ", "Năm thứ", "Nhóm", "Ghi chú", "Ngày gửi"]; const safe = (value: unknown) => { const text = String(value); return /^[\t\r\n ]*[=+\-@]/.test(text) ? `'${text}` : text; }; const csv = [header, ...rows.map(r => [r.registrantName, r.deceasedName, r.relationship, r.prayerYear, r.groupName, r.notes, new Date(r.createdAt).toLocaleDateString("vi-VN")])].map(line => line.map(v => `"${safe(v).replaceAll('"','""')}"`).join(",")).join("\n"); const a = document.createElement("a"); a.href = URL.createObjectURL(new Blob(["\ufeff" + csv], { type: "text/csv" })); a.download = "danh-sach-cau-nguyen.csv"; a.click(); }
  ...
  return <main className="dashboard-shell">
    <aside className="sidebar"><a className="brand light" href="/"><img className="brand-logo" src="/logo.png" alt="Hội Huyền Vi Học" /><span><strong>Hội Huyền Vi Học</strong><small>Lễ Gia Tiên 2026</small></span></a><nav><button className={tab === "registrations" ? "active" : ""} onClick={() => setTab("registrations")}>◫ <span>Danh sách ghi danh</span></button>{isMaster && <button className={tab === "users" ? "active" : ""} onClick={() => setTab("users")}>♙ <span>Người quản lý nhóm</span></button>}</nav><div className="account"><span>{data.user.displayName.slice(0, 1).toUpperCase()}</span><div><b>{data.user.displayName}</b><small>{isMaster ? "Tổng quản lý" : `Nhóm ${data.admin.groupName}`}</small></div></div><button className="signout" onClick={signOut}>Đăng xuất</button></aside>
